...
const PlateWidth   = 3 * BoreholeOffset
const PlateDepth   = 2 * BoreholeOffset
...
const main = () => {
  const CornerXOffset = PlateWidth/2
  const CornerYOffset = PlateDepth/2

  let Plate = union(
    subtract(
      cuboid({ size:[ PlateWidth,PlateDepth,Thickness-2*Epsilon ] }), // main body
  ...
  Plate = translate([ 0,-BoreholeOffset/2,0 ], Plate)
  ...
  for (let i = 0, l = 3; i < l; i++) {
    for (let j = 0, k = 2; j < k; j++) {
      Plate = subtract(Plate, translate(
        [ (i-1)*BoreholeOffset,(j-1)*BoreholeOffset,0 ],
