...
const main = () => {
  ...
  let BracketExtension = BoreholeOffset

  let extendedBracket = cuboid({ size:[ Thickness,BracketLength,ServoBodyWidth+BracketExtension ] })
    extendedBracket = translate([ 0,0,BracketExtension/2 ], extendedBracket)

    for (let i = 0; i < BoreholeCount; i++) {
      extendedBracket = subtract(
        extendedBracket,
        translate(
          [ 0,(i-(BoreholeCount-1)/2)*BoreholeOffset,0 ],
          rotateY(
        ...
        translate(
          [ 0,(i-(BoreholeCount-1)/2)*BoreholeOffset,BoreholeOffset ],
          rotateY(
            Angle90, cylinder({ radius:BoreholeRadius+Epsilon, height:Thickness })
          )
        )
  ...
  extendedBracket = translate([0,BracketLength/2,ServoBodyWidth/2], extendedBracket)


  let leftBracket  = translate([ -0.5*Thickness-ServoFlangeLength,0,0 ], extendedBracket)
  let rightBracket = translate([ 0.5*Thickness+ServoFlangeLength+ServoBodyLength,0,0 ], extendedBracket)


  const BaseLength = ServoBodyLength+2*ServoFlangeLength

  let Base = cuboid({ size:[ BaseLength, Thickness, ServoBodyWidth ] })
    BoreholeCount = Math.floor(BaseLength/BoreholeOffset)

    for (let i = 0; i < BoreholeCount; i++) {
      Base = subtract(
        Base, translate(
          [ (i-(BoreholeCount-1)/2)*BoreholeOffset,0,0 ],
          rotateX(
            Angle90, cylinder({ radius:BoreholeRadius+Epsilon, height:Thickness })
          )
        )
  ...
  Base = translate([ ServoBodyLength/2, BracketLength-Thickness/2, ServoBodyWidth/2 ], Base)
